...
const main = (() => {
	const myFleet = () => {
		const ship4one = gameBoard.positionShip(ship(4), 35, 'vertical');
		const ship3one = gameBoard.positionShip(ship(3), 70, 'vertical');
		const ship3Two = gameBoard.positionShip(ship(3), 82, 'vertical');
		const ship2one = gameBoard.positionShip(ship(2), 43, 'vertical');
